function [ratio,nbCells,Mpercents] = simulateCancer(save2DSnapshots, save3DSnapshots, dishSize, dishHeight, initialNumberOfCells, snapshotSteps,shape,scale,survivalRules, birthRules, pMesen,folder,nbSteps)
    % In :
    ...
    %   * save3DSnapshots         - boolean. set to true if 3D snapshots of
    %                               the dish need to be saved.
    %   * shape                   - shape of the gamma distribution of the
    %                               treatment
    %   * scale                   - scale of the gamma distribution of the
    %                               treatment
    %
    % Out :
    ...
    for i=1:dishSize
        for j=1:dishSize
            ...
            if (rand() < spacingOffset*gamrnd(shape,scale)) && ((((i-dishSize/2)^2 +(j-dishSize/2)^2) < (radius^2)))
                if (rand() < pMesen/100)
                    cells(i,j,1)=1;
                else
                    cells(i,j,1)=2;
    ...
    if(any(snapshotSteps==0))
        ...
        if(save3DSnapshots) % for the 3D plot
            drawAndSave3DPlot(cells, folder,0);
        end
    end
    ...
    for step = 1:nbSteps % main loop
        ...
        for i=1:dishSize^2 % and then we simulate each cell
            for j=1:dishHeight
                nextStepCells = undergoFate(x(i),y(i),j,cells,nextStepCells,survivalRules,birthRules);
            end
        end
        ...
        if(any(step == snapshotSteps))%if step correspond at the time where we want to get picture
            ...
            if(save3DSnapshots) % for the 3D plot
                drawAndSave3DPlot(cells, folder, step);
            end
        end
    end
end
...
function newStepCells = undergoFate(x,y,z,cells, newStepCells,survivalRules,birthRules)
    ...
    elseif(cells(x,y,z)~=0)%else %if there is a cell at grid(x,y)
        if (~any(sum(nNeighbors(dz))-1 == survivalRules) && cells(x,y,z)==2)
            newStepCells(x,y,z) = 0;% '-1' because the current cell is living
        elseif(cells(x,y,z)==1)&&(nNeighbors(z) == 8)
            newStepCells(x,y,z)=2;
        elseif (cells(x,y,z)==1)%movement of the cells
            rp=rand();%random number for the direction of the movement.
            accum=0;
            chance = 1/(8-nNeighbors(z));
            for i=1:8 %2D movement loop
                dx=round(cos(i*pi/4));
                dy=round(sin(i*pi/4));
                if (x+dx < 1)||(x+dx >= size(cells,1))||(y+dy < 1)||(y+dy >= size(cells,2))
                    continue %if the pointed position is outside the dish, we skip it
                end
                if (cells(x+dx,y+dy,z)~=0 && newStepCells(x+dx,y+dy,z)~=0)
                    continue
                end
                if (accum < rp) && (rp < accum+chance)%if it gets the chance to move
                    newStepCells(x+dx,y+dy,z) = cells(x,y,z);%the cell move
                    newStepCells(x,y,z) = 0;%and there is no cell left at the previous emplacement
                    break;
                else
                    accum = accum + chance;%increase the chances of the movement for the next position
                end
            end
        else
